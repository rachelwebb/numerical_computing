...
\objective{Use visualizations of complex functions to estimate their zeros and poles.}
...
From Problem \ref{prob:zeros} you should also have noticed that the plot of $z^3 - i*z^4 - 3*z^6$ looks a lot like the plot of $z^3$ near the origin.
...
% Insert plot of $f(z) = z^3 - i*z^4 - 3*z^6$. From this plot we see that $f(z)$ has a zero of order 3 at the origin, and 3 zeros of order 1 scattered around it. This accounts for the 6 roots of $f(z)$ that are guaranteed to exist by the Fundamental Theorem of Algebra.
